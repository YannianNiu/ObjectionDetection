...
class YoloBody(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        #  backbone
        self.backbone = darknet53(None)  # 获取darknet53网络

        out_filters = self.backbone.layers_out_filters  # 残差块的输出通道数 [64, 128, 256, 512, 1024]
        #  last_layer0
        final_out_filter0 = len(config["yolo"]["anchors"][0]) * (5 + config["yolo"]["classes"])  #3*（5+80）
